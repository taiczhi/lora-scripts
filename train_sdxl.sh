...
network_dropout="0.3" # dropout 是机器学习中防止神经网络过拟合的技术，建议0.1~0.3 
...
output_name="sdxllora"           # output model name | 模型保存名称
...
cache_latents_to_disk=0 #开启缓存潜变量保存到磁盘，这样下次训练不用再次缓存转换，速度更快
...
min_bucket_reso=1024              # arb min resolution | arb 最小分辨率
